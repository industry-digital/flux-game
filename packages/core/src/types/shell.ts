export type ShellPerformanceProfile = {
  naturalPowStat: number;
  naturalFinStat: number;
  naturalResStat: number;
  effectivePowStat: number;
  effectiveFinStat: number;
  effectiveResStat: number;

  /**
   * "Short distance" Gap closing time in seconds.
   * Short distance is about ~10 meters
   */
  gapClosing10: number;

  /**
   * "Long distance" Gap closing time in seconds.
