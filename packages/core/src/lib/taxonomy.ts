...
export const createGroupUrn = (...terms: string[]): AnyGroupURN => createEntityUrn(EntityType.GROUP, ...terms);
...
export const createItemUrn = (itemType: ItemType, ...terms: string[]): ItemURN => `${ROOT_NAMESPACE}:item:${itemType}:${terms.join(':')}`;
export const createWeaponUrn = (...terms: string[]): WeaponItemURN => createItemUrn('weapon', ...terms) as WeaponItemURN;
export const createArmorUrn = (...terms: string[]): ArmorItemURN => createItemUrn('armor', ...terms) as ArmorItemURN;
