...
describe('MaterializeActorCommandHandler', () => {
  ...
  describe('reducer behavior', () => {
    it('should add actor to place entities', () => {
      ...
      const result = materializeActorCommandReducer(context, command as MaterializeActorCommand);
    ...
    it('should declare ACTOR_DID_MATERIALIZE event', () => {
      ...
      materializeActorCommandReducer(context, command as MaterializeActorCommand);

      // Should have declared the materialize event
      expect(context.declareEvent).toHaveBeenCalledWith({
        type: 'actor:materialized',
        actor: actor.id,
        location: place.id,
        payload: {},
        trace: command.id
    ...
    it('should handle actor not found error', () => {
      ...
      expect(context.declareError).toHaveBeenCalledWith('Actor not found in `actors` projection. Did you remember to load it?');
    });
  ...
  describe('handler configuration', () => {
    ...
    it('should use the correct reducer function', () => {
      expect(handler.reduce).toBe(materializeActorCommandReducer);
    });
  });
});
