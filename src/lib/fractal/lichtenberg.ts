/**
 * Geometric Lichtenberg figure generation
 * Optimized implementation with O(N log N) time complexity
 * All impurities are injected for testability and performance
 */
...
export function generateOptimizedLichtenbergFigure(
  ...
  if (config.sparking?.sparkingConditions.boundaryPoints) {
    config.sparking.sparkingConditions.boundaryPoints.forEach((boundaryX: number, index: number) => {
...
function mergeSparkFigureOptimized(
  ...
  const vertexIdSet = new Set(verticesToAdd.map(v => v.id));

  // O(M) connection filtering instead of O(M²)
  const connectionsToAdd = sparkFigure.connections.filter(conn =>
...
// Main function that mirrors the original API
