...
describe('profile', () => {
  ...
  describe('performance characteristics', () => {
    ...
    it('accurately measures relative timing differences', () => {
      const shortWork = () => {
        for (let i = 0; i < 100; i++) {
          Math.sqrt(i);
        }
      ...
      const shortResult = profile(shortWork);
      const longResult = profile(longWork);

      // Long work should take longer than short work
      // (This might be flaky in very fast environments, but generally holds)
      expect(longResult.duration).toBeGreaterThan(shortResult.duration);
      expect(shortResult.result).toBe('short');
      expect(longResult.result).toBe('long');
    });
  });
