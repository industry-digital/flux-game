...
const describeWeatherChange = (
  prev: Weather,
  current: Weather,
  ops: PotentiallyImpureOperations
): string => {
  const initialContext: WeatherReducerContext = {
    ...ops,
    previous: prev,
    narrative: [],
    descriptors: {
      timeOfDay: getTimeOfDay(current.ts),
      mood: 'neutral'
    },
    intensity: 0
  };
  ...
  return description.narrative.join('. ').replace(/\.\./g, '.');
};

// Public API
export const describeWeather: Template<DescribeWeatherProps> = (props, ops) => {
  ...
  return describeWeatherChange(props.previous, props.current, ops);
};
```
...
## 🎨 **Meaningful Atmospheric Transitions**
...
```typescript
// Input: 5:45 AM → 6:15 AM, clear skies
...
- ✅ **Good**: "Heavy rain drums against the ground"
- ❌ **Bad**: "Heavy rain forms puddles that slow movement"
- ✅ **Good**: "A thick fog rolls in, reducing visibility"
- ❌ **Bad**: "Fog appears, reducing your vision range to 2 tiles"

Let the game mechanics handle mechanical effects. The description system focuses on atmospheric immersion.

## 📝 **Implementation Notes**

- **PPFD Integration** - Use PPFD values (0-2000 μmol/m²/s) to drive light-based descriptions
...
```
game/src/template/weather/
├── types.ts              # WeatherDescription, WeatherReducer types
├── reducers/
│   ├── solar.ts          # applySolarGeometry reducer
│   ├── clouds.ts         # applyCloudCover reducer
│   ├── precipitation.ts  # applyPrecipitation reducer
│   ├── narrative.ts      # enhanceNarrative reducer
│   └── final.ts         # renderFinalDescription reducer
├── transitions/
│   ├── meaningful.ts     # meaningfulTransitions definitions
│   └── detection.ts      # transitionDetection logic
├── utils/
│   ├── time.ts           # time-related utilities
│   └── ppfd.ts           # PPFD calculations
├── index.ts              # Main describeWeather function
└── index.spec.ts         # Comprehensive tests for describeWeather
```

### **Migration Tasks**
The existing `describeWeather` function needs to be reorganized and properly implemented:

1. **Move existing code**: `game/src/template/weather.ts` → `game/src/template/weather/legacy.ts` (for reference)
...
```typescript
// ✅ Pure function using refined reducer interface
...
// ❌ Avoid - impure function with hidden dependencies
